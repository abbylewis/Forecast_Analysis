...
Section 4: Determine the most common forecast variables. Do this, we started by visually analyzing the types of forecast variables present, then counted the number in each of six categories using the code below
```{r}
#Load data
complete_forecasts <- read.csv("../Data/complete_dataset_with_source.csv")
...
Section 5: Analyze and plot the relationship between time step and time horizon of forecasting papers
```{r}
#Load data
complete_forecasts <- read.csv("../Data/complete_dataset_with_source.csv")
...
Section 6: Plot and calculate summary statistics for the temporal duration of data used to create a forecasting paper (summed over model development, training, evaluation, etc.) 
```{r}
#Load data
complete_forecasts <- read.csv("../Data/complete_dataset_with_source.csv")

#Calculate summary statistics for plot
mean_dc<- round(mean(as.numeric(complete_forecasts$Data_coverage)/365, na.rm = T),1) #Mean duration
median_dc <- round(median(as.numeric(complete_forecasts$Data_coverage)/365, na.rm = T),1) #Median duration
...
min(as.numeric(complete_forecasts$Data_coverage), na.rm = T) #minimum
max(as.numeric(complete_forecasts$Data_coverage)/365, na.rm = T) #maximum
...
Section 7: Calculate the number of unique journals and conference proceedings, as well as the journal with the most papers represented
```{r}
#Load data
complete_forecasts <- read.csv("../Data/complete_dataset_with_source.csv")
...
Section 8: Best practice analysis. 
```{r}
#Load data
complete_forecasts <- read.csv("../Data/complete_dataset_with_source.csv")
...
#Calculate how many best practices wach paper demonstrated
...
results%>% #Filter out unnecessary variable names
  select(-var2, -var)
results #view final table
...
Section 9: Uncertainty type statistics
```{r}
#Load data
complete_forecasts <- read.csv("../Data/complete_dataset_with_source.csv")
...
complete_forecasts$Uncert_yn <- as.numeric(!complete_forecasts$Uncert_category == "no") #If the forecast has any sort of uncertainty it counts as demonstrating uncertainty
sum(complete_forecasts$Uncert_dd)/nrow(complete_forecasts%>%filter(Uncert_yn == 1))*100 #Percentage of papers that report uncertainty that use data-driven sources
```

Section 10: Forecast evaluation statistics

Over 75% of forecasts that did not report forecast evaluation in the text predicted at forecast horizons of at least one year; in comparison, 52% predicted at forecast horizons of at least one year in the dataset as a whole. As noted in the Methods, the most commonly reported metric of forecast performance was R2.

```{r}

```

Of the papers that mentioned a specific end user (n = 35), 31% mentioned that the end user aided in forecast development and 46% mentioned that forecasts were in use by the end user. 

However, it is rare for a paper to include any explicit mention of ethical considerations made in designing the forecast (5% of papers overall; 25% of forecasts that are in use by an end user)

Section 11: End user statistics and ethical considerations
```{r}
...
Data assimilation for iterative forecasts most often updated initial conditions but not parameters of the model (65% of iterative forecasts only updated initial conditions). 

Section 12: Data assimilation statistics
```{r}
#Load data
complete_forecasts <- read.csv("../Data/complete_dataset_with_source.csv")
...
Section 13: Uncertainty partitioning statistics

all three papers that partitioned uncertainty were published in or after 2014
All of the papers that partitioned uncertainty (n = 3) quantified the influence of process, initial condition, and parameter uncertainty, and only one partitioned driver uncertainty. Process uncertainty dominated total uncertainty for two papers, while initial condition uncertainty dominated in the third paper. 
```{r}
#Load data
complete_forecasts <- read.csv("../Data/complete_dataset_with_source.csv")
...
Section 14: Forecast archiving statistics
For papers that described forecast archiving (n = 15), the most common repository for archived forecasts was Zenodo (used in 20% of papers that archive forecasts); other papers used websites specific to the forecasting project to archive their forecasts. 
```{r}
#Load data
complete_forecasts <- read.csv("../Data/complete_dataset_with_source.csv")
...
Section 15: Null model statistics
Of the papers that used null models in this study (n = 21), 62% used persistence null models and 48% used climatology null models. Two papers used both persistence and climatology null models. 
```{r}
#Load data
complete_forecasts <- read.csv("../Data/complete_dataset_with_source.csv")
