...
r2s$Fish <- 0
r2s$Fish[grepl("anchovy|mackarel|sardine|walleye|tuna|bonito|hake|fish",r2s$Var, ignore.case = T)]<-1
...
r2s%>%
  ...
  ggplot(aes(x = as.numeric(Horiz_days), y = as.numeric(R2_value), color = Met_covar_yn))+
  geom_point()+
  xlim(0,10)+
  geom_smooth()
...
bar%>%
  ...
  ggplot(aes(x = name, y = mean, fill = as.factor(value)))+
  ...
  geom_errorbar(aes(ymin = mean-sd, ymax = mean+sd, group = as.factor(value)), position = "dodge")+
  geom_text(aes(label = n_papers), position = "dodge")
