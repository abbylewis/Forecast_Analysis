...
dups$How_often_days[dups$How_often_days%in%c("0","never","UNK")]<-NA
dup2 = dups%>%
  group_by(Title)%>%
...
for_dup_compare <- as.data.frame(t(dup_include1%>%select(-forecast_agree,-reviewers_diff,-Round,-Author_n)))
write.csv(for_dup_compare,"for_dup_compare.csv")
```
```{r}
