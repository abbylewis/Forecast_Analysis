...
new_names <- c("PaperID", "Reviewer","Title","Authors","DOI","Journal",
                    ...
                    "Automated_yn","Archiving_yn","Repository","Drivers_published","Header5",
                    "End_user_yn","End_user_partnership","Used_by_stakeholder_yn","End_user_type",
...
Journals
```{r}
jpeg("EF_journals.jpg",res=300,width = 4, height = 10, units = "in")
complete_forecasts%>%
  ggplot(aes(y = Journal, fill = as.factor(Round)))+
  geom_bar()+
  labs(fill = "Round")
dev.off()
complete_forecasts$Round
```
...
```{r}
n_breaks = 2
breaks <- quantile(as.numeric(complete_forecasts$Year), probs = seq(0,1,length.out = n_breaks+1)) #to make this divided in three use probs = c(0,1/3,2/3,1)
breaks <- floor(breaks)
#breaks = c(1932,2000,2016,2020)
for(i in n_breaks:1){
  complete_forecasts$Quantile[complete_forecasts$Year <= breaks[i+1]] <- paste0(breaks[i]+1, " to ", breaks[i+1])
}

complete_forecasts$Archiving_yn[complete_forecasts$Archiving_yn=="No data streams"] <- 0
complete_forecasts$Forecast_eval_shown[complete_forecasts$Forecast_eval_shown == "y (very loosely)"] <- 1
...
time_graph2$name = factor(time_graph2$name, levels = c("Uncert_yn","Iterative_yn","End_user_yn","Null_yn","Archiving_yn","Eval_yn", "Proc"))

jpeg("Forecasts_over_time.jpeg",width = 8, height = 4, units = "in", res = 300)
time_graph2%>%
  ggplot(aes(x = name, y = Percent, fill = Quantile))+
  geom_bar(stat = "identity", position = "dodge", color = "black")+
  xlab("")+
  ylab("Percent of papers")+
  scale_x_discrete(breaks = c("Uncert_yn","Iterative_yn","End_user_yn","Null_yn","Archiving_yn","Eval_yn", "Proc"), labels = c("Data-driven \nuncertainty", "Iterative \nforecasts","End user \nspecified","Null model \ncomparison","Forecast \narchiving","Forecast \nevaluation \nin paper", "Process-based \nmodel"))+
  theme_light()+
...
Harris best practices
Compare multiple modeling approaches
```{r}
complete_forecasts%>%
  ggplot(aes(x = Multiple_approaches_yn))+
  geom_bar()
...
complete_forecasts%>%
  ...
  ggplot()+
  geom_histogram(aes(x = as.numeric(Data_coverage)/365, fill = Quantile),bins = 10)+
...
time_bin[!is.na(time_bin$Time_step_days)&time_bin$Horiz_days<time_bin$Time_step_days,]
```

Pay attention to uncertainty 
```{r}
complete_forecasts$Iterative_yn[complete_forecasts$Iterative_yn == "from the perspective of the weather forecast, not the pollen forecast - so n"] <- "0"
complete_forecasts %>%
...
Address unknown or unmeasured predictors 
```{r}

```

Assess how forecast accuracy changes with time-lag 
```{r}
...
Make both short-term and long-term predictions
```{r}
