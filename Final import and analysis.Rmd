...
jpeg("Pubs_year_noEco.jpeg", width = 7, height = 6, units = "in",res = 300)
complete_forecasts%>%
  ggplot()+
...
data7 <- read_csv("papers_citing_forecasts_full.csv")
data8 <- read_csv("referenced_papers_forecasts.csv")
data9 <- read_csv("citing_ed_foundLate.csv")%>%
...
jpeg("EF_journals.jpg",res=300,width = 20, height = 12, units = "in")
journals%>%
  ggplot(aes(y = SO))+
  geom_bar()+
...
complete_forecasts[complete_forecasts$Ecosystem == "NA",]
...
map <- ggplot() +
  ...
  geom_point(data = for_map2%>%filter(Region == 1), aes(Long, Lat, fill = Ecosystem), shape = 21, color = "black", show.legend = F, size = 5, alpha  =.3)+
  geom_point(data = for_map2%>%filter(Region == 0), aes(Long, Lat, fill = Ecosystem), shape = 21, color = "black", show.legend = F, size = 3, alpha  =1)+
...
class <- complete_forecasts%>%
  ...
  geom_bar()+
  scale_fill_manual(breaks = c("agricultural","desert","grassland","forest","atmosphere","freshwater","marine","tundra","urban","other","NA"), values = colors, na.translate = T, na.value = "grey50", name = "Ecosystem",guide = guide_legend(ncol = 2))
...
results = data.frame(var2 = "Include data driven uncertainty")%>%
  full_join(data.frame(summary(unc)$coefficients, var2 = NA))%>%
  ...
  select(var2, param, Estimate, Std..Error, z.value, Pr...z..)
...
jpeg("best_practices)regression.jpg",res = 300,width = 8, height = 6, units = "in")
plot = prob_df%>%
