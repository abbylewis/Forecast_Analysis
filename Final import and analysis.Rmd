...
comb <- read_csv(paste0("Finished_dups/",files2[1]), col_types = paste0(rep("c",82),collapse = ""), col_names = new_names, skip = 1)

for (i in 2:length(files2)){
  new <- read_csv(paste0("Finished_dups/",files2[i]), col_types = paste0(rep("c",82),collapse = ""), col_names = new_names, skip = 1)
  comb = comb%>%
    full_join(new)
}
...
plot(for_hist$Year, for_hist$total, main = "",xlab = "Year", ylab = "ecological forecasts", type = "l", ylim = c(0,for_max))
par(new = T)
...
length(unique(complete_forecasts$Journal)) #96 different journals
```
