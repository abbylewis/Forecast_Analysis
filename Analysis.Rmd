...
#data[data$NearTerm==1&data$Forecast==1&data$Ecological==0,]
data$Forecast[!is.na(data$Only.Space)]<-0 #Change if we are keeping spatial forecasts
to_venn <- data%>%
  filter(nchar(PaperUnavailable) == 0)%>%
